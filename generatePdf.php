...
if (isset($_SESSION['selectedDate'])) {
    ...
    // Remove any extra page break at the very end
    $html = preg_replace('/<pagebreak \/>$/', '', $html);
    $mpdf->WriteHTML($html);

    // Define folder path using current date
    ...
    $pdfFileName = date('d-m-Y_H-i') . 'Report.pdf';
    $pdfFilePath = $fullPath . $pdfFileName;
...
$sheet->setBreak('M1', \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_COLUMN);
...
function renderTable1($sheet, $conn, $sql, $title, $startRow, $date,$table1r,$table2r,$table3r,$table4r) {
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        foreach ($columns as $colIndex => $col) {
            ...
            if(is_numeric($value) && !($value==="OFF" || $value==="ON"))
            {
...
$sql1 = "WITH ABC AS (
            SELECT *,
                ROW_NUMBER() OVER (
                    PARTITION BY DATEDIFF(MINUTE, '$selectedDate $ftime', [timestamp]) / $itime
...
function renderTable2($sheet, $conn, $sql, $title, $startRow, $date) {
    ...
    foreach (sqlsrv_field_metadata($stmt) as $field) {
        ...
        if (stripos($displayName, "FCV_WFE_6501_OPENING") !== false) {$displayName = "FCV-HWR-6501";}
...
<body>
    <div class="container">
        <?php if (file_exists($pdfFilePath) && (file_exists($excelFilePath))) { ?>
            <p class="success-msg">PDF & Excel created successfully!</p>
            <div class="file-path"><?php echo "PDF:".$pdfFilePath; ?>
                <?php echo "<br>EXCEL:".$excelFilePath; ?>
            </div>
        <?php } ?>
    </div>
</body>
